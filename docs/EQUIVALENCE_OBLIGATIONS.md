...
`MacroSlice.lean`, which is the current macro-generated contract surface. 7/18 operations are
macro-migrated; the remaining 11 are blocked on upstream macro
...
The 11 unmigrated operations depend on upstream verity macro capabilities.
...
At `a026df14`, the repo can now use tuple params in executable `verity_contract`
definitions, `MappingStruct`/`MappingStruct2`, `internalCall`, `mstore`/`mload`,
and macro-backed `ecrecover`. The remaining gaps are now narrower and mostly concern
...
|-----------|----------------|:------:|:-------------------------------:|--------|
| `setOwner` | getStorageAddr, setStorageAddr, msgSender, require | **PROVEN** | **PROVEN** | available at verity pin a026df14 |
| `setFeeRecipient` | getStorageAddr (×2), setStorageAddr, msgSender, require | **PROVEN** | **PROVEN** | available at verity pin a026df14 |
| `enableIrm` | getMapping, setMapping, getStorageAddr, msgSender, require | **PROVEN** | **PROVEN** | available at verity pin a026df14 |
...
At verity pin `a026df14`, Link 2 is tracked on the typed-IR semantic bridge path
...
### Discharge sequence (current pin: `a026df14`)
