/-
  ...
  (morpho-blue-verity/Morpho/Contract.lean) reads, together with that predicate transcribed verbatim.
...
/-- Outstanding borrowed assets attributed to the position, rounded up.
    Mirrors `borrowedAmt` at morpho-blue-verity/Morpho/Contract.lean:577. -/
...
/-- Maximum borrow the collateral supports at the current price, rounded down.
    Mirrors `maxBorrow` at morpho-blue-verity/Morpho/Contract.lean:579-580. -/
...
/--
  ...
  `borrowShares == 0` short-circuit in `_isHealthy` (morpho-blue-verity/Morpho/Contract.lean:585-586):
