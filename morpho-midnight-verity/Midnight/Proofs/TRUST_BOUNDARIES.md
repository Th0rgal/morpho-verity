...
| `midnight_idlib_toid_market_abi_create2_preimage`, `midnight_sstore2_market_initcode_layout`, `create2_address_derivation`, `midnight_sstore2_market_runtime_abi_decode`, `midnight_sstore2_market_runtime_abi_return` | `IdLib.toId`, `touchMarket`, and `toMarket` CREATE2/SSTORE2 **market address derivation**. The SSTORE2/CREATE2 is used as a CREATE2 factory for deterministic market addresses; market data itself lives in regular storage (`marketStateSlot`). Verity's typed `CodeData` API (added in #1971) targets a different use case (storing arbitrary data in SSTORE2) and does not replace this market-address-factory pattern. | 
