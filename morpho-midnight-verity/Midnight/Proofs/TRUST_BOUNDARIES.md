...
| `midnight_idlib_market_initcode_abi_hash`, `midnight_sstore2_market_initcode_layout`, `midnight_sstore2_market_runtime_abi_decode`, `midnight_sstore2_market_runtime_abi_return`; plus Verity `Create2SSTORE2.deployModule` assumptions `create2_initcode_layout` and `create2_address_derivation` | `IdLib.toId`, `touchMarket`, and `toMarket` CREATE2/SSTORE2 **market address derivation**. The SSTORE2/CREATE2 path is used as a CREATE2 factory for deterministic market addresses; market data itself lives in regular storage (`marketStateSlot`). The older Midnight-local preimage ECM has been replaced by the narrower market-initcode ABI hash ECM and the shared Verity CREATE2 module. This consolidates the boundary; it does not eliminate the CREATE2 derivation trust surface. Verity's typed `CodeData` API (added in #1971) targets a different use case (storing arbitrary data in SSTORE2) and does not replace this market-address-factory pattern. | The generated Midnight body now emits the source-shaped initcode construction and uses the shared CREATE2 helper, but dynamic market payload layout, CREATE2 address derivation, runtime decoding, and dynamic return ABI framing remain byte-level obligations. |
