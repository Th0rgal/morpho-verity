...
| `SafeTransferLib.safeTransfer` | Verity `Compiler.Modules.ERC20.legacyStringSafeTransferModule` ECM call | External-call trust boundary; model delegates ERC20 mechanics to the Verity-owned module (custom `MorphoSafeTransfer` ECM removed). | Covered empirically through original tests; parity run green. |
| `SafeTransferLib.safeTransferFrom` | Verity `Compiler.Modules.ERC20.legacyStringSafeTransferFromModule` ECM call | External-call trust boundary; model delegates ERC20 mechanics to the Verity-owned module (custom `MorphoSafeTransfer` ECM removed). | Covered empirically through original tests; parity run green. |
